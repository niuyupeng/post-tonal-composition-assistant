...
The default dataset is synthetic, so it tests constraint satisfaction and pipeline reproducibility rather than style imitation from copyrighted repertoire. The Transformer is intentionally modest for local training. Constraint-guided decoding uses reranking over symbolic candidates for non-differentiable metrics; those penalties are not reported as fake training losses.
