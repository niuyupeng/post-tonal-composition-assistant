...
This blinded package contains {len(items)} score-level MusicXML examples generated by one fixed score-generation configuration. Every part has the requested number of measures, and the requested part count has been verified. Generator identity and automatic metric values are withheld from raters.
...
def prepare_from_evaluation_examples(
    ...
    selected = [
        ...
        and item.get("musicxml_structurally_valid", False)
        and item.get("musicxml_measure_count_adherent", False)
        and item.get("musicxml_voice_count_adherent", False)
    ][:count]
