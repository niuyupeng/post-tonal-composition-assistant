...
For cross-checkpoint replication, the identical 2,000-condition protocol is run separately with the seed-42, seed-43, and seed-44 checkpoints. Generation uses batches of 32 under the same per-item seed schedule. Every per-sample record stores its condition identifier, checkpoint provenance, sampling protocol, candidate count, and first-candidate SHA-256 fingerprint. Aggregation is permitted only when all 2,000 condition identifiers, subset masks, endpoint values, and first-candidate fingerprints pass the alignment gate for every checkpoint.
